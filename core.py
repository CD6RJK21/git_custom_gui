...
def output_to_plain_text(object, messege):
    object.console_output.appendPlainText(messege + '\n' + ('-' * 88) + '\n')

def execute(object, command):
    import subprocess
    try:
        output_to_plain_text(object, '>>{}\n'.format(command) +
                             str(subprocess.check_output(command.split()), encoding='utf-8'))
